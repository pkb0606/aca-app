...
def is_legacy_hash(stored: str) -> bool:
    # legacy: plain sha256 hex digest (64 chars)
    return bool(re.fullmatch(r"[0-9a-f]{64}", (stored or "").strip()))
...
def verify_password(password: str, stored_hash: str) -> bool:
    """Verify password against either PBKDF2 hashes or legacy SHA256 hashes."""
    ...
    # legacy fallback (do not create new hashes in this format)
...
def validate_password_strength(pw: str) -> tuple[bool, str]:
    ...
    - length >= 10
    - no spaces
    - at least 3 of 4 categories: lower, upper, digit, special
    """
    if pw is None:
    ...
    if any(ch.isspace() for ch in pw):
        return False, "비밀번호에 공백을 넣을 수 없습니다."

    cats = 0
    cats += 1 if re.search(r"[a-z]", pw) else 0
    cats += 1 if re.search(r"[A-Z]", pw) else 0
    cats += 1 if re.search(r"[0-9]", pw) else 0
    cats += 1 if re.search(r"[^A-Za-z0-9]", pw) else 0

    if cats < 3:
        return False, "영문 소문자/대문자/숫자/특수문자 중 3종류 이상을 포함하세요."
    return True, ""
...
def _get_table_columns(table_name: str):
    """SQLite 테이블의 컬럼명 리스트 반환. 테이블이 없으면 빈 리스트."""
    conn = get_connection()
    cur = conn.cursor()
    try:
...
def ensure_attendance_schema():
    """구버전 DB(컬럼 누락)에서도 앱이 죽지 않도록 attendance 스키마를 보정."""
    cols = set(_get_table_columns("attendance"))
    if not cols:
        # 테이블 자체가 없으면 init_db가 만들도록 하고 다시 확인
        init_db()
        cols = set(_get_table_columns("attendance"))
        if not cols:
            return
    ...
    if "homework_status" not in cols:
        add_cols.append(("homework_status", "TEXT"))
    if "daily_test_status" not in cols:
        add_cols.append(("daily_test_status", "TEXT"))
    if "checkin_time" not in cols:
        add_cols.append(("checkin_time", "TEXT"))
    if "via" not in cols:
        add_cols.append(("via", "TEXT"))
    if "recorded_by" not in cols:
        add_cols.append(("recorded_by", "INTEGER"))
    ...
    if add_cols:
        ...
        for c, typ in add_cols:
            try:
                cur.execute(f"ALTER TABLE attendance ADD COLUMN {c} {typ}")
            except sqlite3.OperationalError:
                # 이미 있거나, 다른 이유(읽기전용 등)면 무시하고 진행
                pass
        conn.commit()
        conn.close()


def get_recent_attendance_for_student_safe(student_id: int, limit: int = 100):
    """attendance 컬럼 누락/구버전 DB에서도 안전하게 최근 출결을 가져온다."""
    ensure_attendance_schema()
    cols = _get_table_columns("attendance")
    # 조회 가능한 컬럼만 선택
    wanted = ["date", "checkin_time", "status", "homework_status", "daily_test_status"]
    ...
    cur = conn.cursor()
    try:
        cur.execute(q, (student_id, limit))
        rows = cur.fetchall()
    except sqlite3.OperationalError:
        # 마지막 방어: 스키마 한번 더 보정 후 재시도
        ensure_attendance_schema()
        cols = _get_table_columns("attendance")
        select_cols = [c for c in wanted if c in cols]
        if not select_cols:
...
def admin_student_management():
    ...
    with tab_view:
        if not students:
            ...
            # 최근 출결 100개
            att_rows, att_cols = get_recent_attendance_for_student_safe(sid, limit=100)
            ...
            if not att_rows:
                ...
                att_data = []
                for row in att_rows:
                    rec = dict(zip(att_cols, row))
                    att_data.append(
                        {
                            "날짜": rec.get("date", ""),
                            "시간": rec.get("checkin_time", "") or "",
                            "출결": rec.get("status", "") or "",
                            "과제": rec.get("homework_status", "") or "",
                            "일일테스트": rec.get("daily_test_status", "") or "",
                        }
                    )
                ...
                subset_cols = [c for c in ["출결", "과제", "일일테스트"] if c in df_att.columns]
                if subset_cols:
                    styled = df_att.style.applymap(color_cell, subset=subset_cols)
...
def admin_lesson_management():
    """
    수업 관리:
    - 반별 수업 관리 (이전 진도/숙제 + 오늘 진도/다음 숙제 + 반 단위 출석)
    - 진도 관리 (전체)  -> 기존 admin_academy_progress 재사용
    - 출석 관리 (일별)  -> 기존 admin_attendance_management 재사용

    ⚠️ Streamlit 탭은 중첩 사용 시 화면이 비거나 깨질 수 있어서,
    상단 라디오로 섹션을 전환하도록 구성한다.
    """
...
def student_password_change():
    ...
    (Menu name kept for backward compatibility.)
    """
    st.markdown("### 🔐 비밀번호 변경")

    user = st.session_state.get("user")
    if not user:
        st.error("로그인 후 사용할 수 있습니다.")
        return
